...
func bindWarriorAction(
	...
	if err := admitWarriorLoadout(fixture, combat.ActionLoadout{
...
func admitWarriorLoadout(fixture *effectIntegrationFixture, loadout combat.ActionLoadout) error {
	var level uint32
	var health, maxHealth int32
	if err := fixture.zone.GameCommand(func(tick gametypes.Tick) error {
		entity := tick.Entity(fixture.playerID)
		if entity == nil {
			return gametypes.ErrUnknownEntity
		}
		level, health, maxHealth = entity.Level, entity.Health, entity.MaxHealth
		return nil
	}); err != nil {
		return err
	}
	abilityIDs := make([]string, 0, len(loadout.Bindings))
	known := make(map[string]struct{}, len(loadout.Bindings))
	for _, binding := range loadout.Bindings {
	...
	return fixture.combat.Admit(fixture.playerID, combat.PlayerAdmission{
		Level:          level,
		Health:         health,
		MaxHealth:      maxHealth,
		AbilityIDs:     abilityIDs,
		ActionBindings: loadout.Bindings,
		ActionResource: loadout.Resource,
	})
}
...
func TestExtractedSharedCooldownBlocksSwitchingActions(t *testing.T) {
	...
	if err := admitWarriorLoadout(fixture, combat.ActionLoadout{Bindings: bindings}); err != nil {
		t.Fatalf("AdmitWithLoadout() error = %v", err)
	}
