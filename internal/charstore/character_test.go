...
// templates is a [charstore.ChargenTemplates] over a map, standing in for what the
// shard builds from the pack's chargen table.
type templates map[string]charstore.Snapshot

func (set templates) Template(chargenOptionID string) (charstore.Snapshot, bool) {
...
func testTemplates() templates {
	return templates{testOption: {
		State: charstore.CharacterState{
			Position: charstore.Vec3{X: 12, Y: 4.5},
			Heading:  1.5,
			Level:    1,
			Health:   100,
		},
...
func newCharacterService(t *testing.T) (*charstore.CharacterService, charstore.Repository) {
	t.Helper()
	repository := charstore.NewMemory()
	worker := charstore.NewSaveWorker(repository, slog.New(slog.DiscardHandler), 8, 0)
	return charstore.NewCharacterService(repository, testTemplates(), worker, slog.New(slog.DiscardHandler), 0), repository
}
...
func TestLoadMaterializesAFirstLoginFromTheChargenTemplate(t *testing.T) {
	...
	if snapshot.State.Position != (charstore.Vec3{X: 12, Y: 4.5}) {
...
func TestLoadReturnsTheSavedStateRatherThanTheTemplate(t *testing.T) {
	...
	moved := first
	moved.State.Position = charstore.Vec3{X: 51.9, Y: 4.5}
	moved.State.SaveSeq = first.State.SaveSeq + 1
	if err := charstore.SaveCharacter(t.Context(), repository, moved); err != nil {
...
func TestLoadRefusesAnUnknownChargenOption(t *testing.T) {
	...
	if !errors.Is(err, charstore.ErrNoTemplate) {
...
func TestCheckpointEnqueuesWithoutWaiting(t *testing.T) {
	...
	repository := charstore.NewMemory()
	worker := charstore.NewSaveWorker(repository, slog.New(slog.DiscardHandler), 1, 0)
	service := charstore.NewCharacterService(repository, testTemplates(), worker, slog.New(slog.DiscardHandler), 0)
	characterID := uuid.New()
