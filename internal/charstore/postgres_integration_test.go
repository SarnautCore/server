...
func newIntegrationRepository(t *testing.T) charstore.Repository {
	t.Helper()
	pool := migratedPool(t)
...
func TestDuplicateCharacterNameIsRejectedByTheDatabaseConstraint(t *testing.T) {
	...
	if err := repository.CreateCharacter(ctx, newCharacter(account.AccountID, "Anne")); !errors.Is(err, charstore.ErrNameTaken) {
...
func TestNormalizedNameUniquenessIsCaseInsensitive(t *testing.T) {
	...
	if err := repository.CreateCharacter(ctx, second); !errors.Is(err, charstore.ErrNameTaken) {
...
func TestRunInTxRollbackIsInvisibleToOtherConnections(t *testing.T) {
	...
	err := repository.RunInTx(ctx, func(ctx context.Context, tx charstore.Repository) error {
		...
		if err := tx.UpsertQuestState(ctx, characterID, charstore.QuestState{
