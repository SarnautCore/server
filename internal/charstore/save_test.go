...
func snapshotFor(characterID uuid.UUID, saveSeq int64) charstore.Snapshot {
...
func TestSaveWorkerDropsWhenTheQueueIsFullInsteadOfBlocking(t *testing.T) {
	...
	const capacity = 2
	worker := charstore.NewSaveWorker(charstore.NewMemory(), discardLogger(), capacity, time.Second)
...
func TestSaveWorkerCountsSupersededSavesSeparately(t *testing.T) {
	...
	if err := charstore.SaveNow(t.Context(), repository, snapshotFor(characterID, 5), time.Second); err != nil {
