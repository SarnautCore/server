...
func testOptions(t *testing.T) (Options, *testClock) {
	...
	clock := newTestClock()
	return Options{
		Repository: store.NewMemoryWithClock(clock.Now),
...
func TestCreateCharacterRejectsADisallowedOptionAndADuplicateName(t *testing.T) {
	...
	for _, duplicate := range []string{"Anne", "An-ne", "A'nne"} {
		if _, err := service.CreateCharacter(ctx, accountID, duplicate, "chargen.league.warrior"); !errors.Is(err, store.ErrNameTaken) {
	...
	if _, err := service.CreateCharacter(ctx, other, "Anne", "chargen.league.warrior"); !errors.Is(err, store.ErrNameTaken) {
...
func TestListCreateDeleteRoundTrip(t *testing.T) {
	...
	if _, err := service.CreateCharacter(ctx, accountID, "Anne", "chargen.league.warrior"); !errors.Is(err, store.ErrNameTaken) {
