...
func newIntegrationService(t *testing.T) *account.Service {
	...
	catalogue, err := account.NewCatalogue(loadFixtureChargen(t))
	if err != nil {
		t.Fatalf("NewCatalogue() error = %v", err)
	}
	service, err := account.New(account.Options{
		Repository: repository,
		Keys:       account.NewMemoryKeyValue(),
		Catalogue:  catalogue,
...
func TestRegisterLoginCreateListDeleteAgainstPostgres(t *testing.T) {
	...
	if _, err := service.Register(ctx, secret.New(strings.ToUpper(email.Reveal())), password); !errors.Is(err, store.ErrEmailTaken) {
	...
	if _, err := service.Login(ctx, email, secret.New("wrong-password")); !errors.Is(err, account.ErrInvalidCredentials) {
	...
	if _, err := service.CreateCharacter(ctx, accountID, name, options[0].ID); !errors.Is(err, store.ErrNameTaken) {
		t.Errorf("CreateCharacter(duplicate) error = %v, want ErrNameTaken", err)
	}
	punctuated := string(name[0]) + "'" + name[1:]
	if len(punctuated) <= account.MaximumNameLength {
		if _, err := service.CreateCharacter(ctx, accountID, punctuated, options[0].ID); !errors.Is(err, store.ErrNameTaken) {
...
func TestAnotherAccountCannotSeeOrTakeACharacterAgainstPostgres(t *testing.T) {
	...
	if _, err := service.MintTicket(ctx, accountB, character.CharacterID); !errors.Is(err, account.ErrCharacterNotFound) {
		t.Errorf("MintTicket(other account) error = %v, want ErrCharacterNotFound", err)
	}
	if err := service.DeleteCharacter(ctx, accountB, character.CharacterID); !errors.Is(err, account.ErrCharacterNotFound) {
