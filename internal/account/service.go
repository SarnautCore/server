// Package account is the auth service: registration, login, the account's
...
func (service *Service) ListCharacters(ctx context.Context, accountID uuid.UUID) ([]store.Character, error) {
...
func (service *Service) CreateCharacter(
	...
	if !ValidName(name) {
		return store.Character{}, fmt.Errorf("%w: %q", ErrNameInvalid, name)
	}
	normalized := NormalizeName(name)
	if blockedName(service.blocklist, normalized) {
		return store.Character{}, fmt.Errorf("%w: %q", ErrNameBlocked, name)
	}
	...
	if err != nil {
		return store.Character{}, fmt.Errorf("account: mint character id: %w", err)
	}
...
func (service *Service) CheckName(ctx context.Context, accountID uuid.UUID, name string) (NameCheck, error) {
	...
	err = service.repository.ReserveName(ctx, store.NameReservation{
		NameNormalized: normalized,
		AccountID:      accountID,
		ReservedUntil:  until,
	})
