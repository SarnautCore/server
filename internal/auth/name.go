...
//
// The normalization itself lives in `internal/charstore`, next to the column whose
// behaviour it has to match; this is the name the rest of the service uses.
func NormalizeName(name string) string { return charstore.NormalizeCharacterName(name) }
