...
func (service *Service) handleRegister(writer http.ResponseWriter, request *http.Request) {
	...
	case errors.Is(err, charstore.ErrEmailTaken):
...
func characterDocuments(characters []charstore.Character) []characterDocument {
...
func (service *Service) handleCreateCharacter(writer http.ResponseWriter, request *http.Request, accountID uuid.UUID) {
	...
	case errors.Is(err, charstore.ErrNameTaken):
	...
	default:
		writeJSON(writer, http.StatusCreated, characterDocuments([]charstore.Character{character})[0])
	}
}
