...
func TestAdmitWithActionsUsesOnlyAuthoredBindings(t *testing.T) {
	...
	if err := fixture.admitWithActions(fixture.playerID, bindings); err != nil {
...
func TestAdmitWithExplicitEmptyBindingsDoesNotInventSlots(t *testing.T) {
	fixture := newHarness(t, extendedPack, targetMob, zeroVector, combat.Options{})
	if err := fixture.admitWithActions(
		fixture.playerID, make([]combat.ActionBinding, 0),
	); err != nil {
		t.Fatalf("AdmitWithActions() error = %v", err)
	}
	bar, err := fixture.module.ActionBar(fixture.playerID)
	...
	for index, slot := range bar.Slots {
		if slot.AbilityID != "" || slot.Available ||
			slot.UnavailableReason != combat.ActionUnavailableEmptySlot {
			t.Fatalf("explicitly empty authored slot %d = %+v", index, slot)
		}
	}
	if _, err := fixture.module.UseAbility(fixture.playerID, combat.AbilityRequest{
		TargetID: fixture.mobID, AbilityID: baseAbility,
	}); err != nil {
		t.Fatalf("unbound granted ability was not retained: %v", err)
	}
}
...
func TestAdmitWithActionsRejectsInvalidBindingsBeforeMutation(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			if err := fixture.admitWithActions(fixture.playerID, test.bindings); !errors.Is(err, test.want) {
...
func authoredActionHarness(t *testing.T) *harness {
	t.Helper()
	fixture := newHarness(t, extendedPack, targetMob, zeroVector, combat.Options{})
	if err := fixture.admitWithActions(fixture.playerID, []combat.ActionBinding{
		{SlotIndex: 0, AbilityID: baseAbility},
	}); err != nil {
		t.Fatalf("AdmitWithActions() error = %v", err)
	}
